<!-- ═══════════════════════════════════════════════════════════════════════════
     ...
     3. Update the checkboxes by editing THIS file:
          - [x]  → - [x]   ONLY when the step is genuinely done and correct in the code.
...
- [x] **✅ Module 1 done when:** create a `book`, call `book.showHeader()` then `bk.showInfor()` → clean aligned table; and you can explain *why fields are private*.
...
- [x] **✅ Module 3 done when:** a one-member table prints; and you can answer *"why no polymorphism for Member but yes for book?"* (one member kind vs. four book kinds).
...
- [x] **Step 4.3 — The 3 `static final` constants**
...
- [x] **Step 4.4 — Two constructors** (no-arg + full 5-arg). ▸ Check: both compile.

- [x] **Step 4.5 — Getters/setters**
  - ▸ Do: getters for all 5 + setters for all 5.
  - ▸ Trap: the boolean getter is **`isReturned()`**, not `getReturned()`.
  - ▸ Check: all compile.

- [x] **Step 4.6 — `getDueDate()`**
...
- [x] **Step 4.7 — `tinhNgayQuaHan(LocalDate ngayTra)`: compute the raw gap**
...
- [x] **✅ Module 4 done when:** the 4.7/4.8/4.9 test cases pass, and you can explain *why the fine logic lives here, not in `LAB01`* (single responsibility — the record owns its rules).
...
- [x] **Step 5.3 — `main` + a temporary `showMenu()` stub**
...
- [x] **Step 5.7 — `showMenu()`: the loop skeleton**
  - ▸ Do: replace the stub with `int choice; do { ... } while (choice != 5);`.
  - ▸ Check: compiles.

- [x] **Step 5.8 — `showMenu()`: print the 5 options + read choice**
...
- [x] **Step 5.9 — `showMenu()`: the `switch`**
...
- [x] **Step 5.10 — Constructor: hardcode the 4 books**
...
- [x] **Step 5.11 — Constructor: hardcode the 3 members**
  - ▸ Do: `Member m01 = new Member("M01","Nguyen Van A","vana@gmail.com","0901111111");` … `m02`, `m03`; then `memberList.add(m01);` …
  - ▸ Check: 3 members.

- [x] **Step 5.12 — Constructor: hardcode the borrow records (test scenarios)**
...
- [x] **✅ Module 5 done when:** app launches with the hardcoded starter data, menu navigates and exits, bad input never crashes it. Unlike a seed helper, this compiles and runs **standalone** — no dependency on Module 6.
...
- [x] **Step 6.1 — `timSachTheoID(String bookID)`** (write FIRST — many things need it)
...
- [x] **Step 6.3 — `findBook(String bookID)`**
...
- [x] **Step 6.4 — `addBook()`: choose the type**
...
- [x] **Step 6.9 — `deletedBook`: remove by index**
...
- [x] **Step 6.11 — Wire into `showMenu()`** — case 1 → `showMenuBook()`.
- [x] **✅ Module 6 done when:** add (incl. duplicate rejection), list, find, and safe-delete all work end to end.
...
- [x] **Step 7.1 — `findMemberByID(String memberID)`** — loop, return `Member` or `null` (`equalsIgnoreCase`).
- [x] **Step 7.2 — `showListMember()`** — empty check → message; else `Member.showHeader()` + loop rows.
- [x] **Step 7.3 — `addMember()`: create + fill** — `Member mb = new Member(); mb.inputInfor();`.
- [x] **Step 7.4 — `addMember()`: reject duplicate + store** — `if (findMemberByID(mb.getMemberID()) != null) {...return;}` else add + success.
- [x] **Step 7.5 — `deletedMember(String memberID)`: guard** — block if a record matches this member and `!isReturned()`.
- [x] **Step 7.6 — `deletedMember`: remove by index** — same index-loop pattern as 6.9.
- [x] **Step 7.7 — `showMenuMember()`** — submenu.
...
- [x] **Step 7.8 — Wire into `showMenu()`** — case 2.
- [x] **✅ Module 7 done when:** add/list/find/safe-delete work; deleting a borrowing member (M01/M02) is blocked.
...
- [x] **Step 8.1 — Empty-storage + read member**
...
- [x] **Step 8.2 — Enforce the borrow limit**
...
- [x] **Step 8.3 — Read book + existence + stock**
...
- [x] **Step 8.4 — Block double-borrow of the same book**
...
- [x] **Step 8.9 — Read return date + confirm** (same pattern as 8.5).

- [x] **Step 8.10 — Commit the return**
...
- [x] **Step 8.11 — Compute & print the fine**
...
- [x] **Step 8.12 — `xemDanhSachDangMuon()`** — loop `borrowList`; for each `!isReturned()` print header once (a `headerShown` flag) + `showInfor()`. None → "No books are currently being borrowed."
- [x] **Step 8.13 — `lichSuMuonTheoMember(String memberID)`** — validate member; loop printing all their records (header once); none → "No borrowing history yet."
- [x] **Step 8.14 — `showMenuBorrow()`** — submenu (1 Borrow, 2 Return, 3 Display borrowed, 4 History by member, 5 Return). Case 4 reads the ID then calls 8.13.
- [x] **Step 8.15 — Wire into `showMenu()`** — case 3.

### 8D — Reports

- [x] **Step 8.16 — `xemSachQuaHan()`**
...
- [x] **Step 8.17 — `showPopularBooks()` pass 1: find the max**
...
- [x] **Step 8.18 — `showPopularBooks()` pass 2: print all at max**
...
- [x] **Step 8.19 — `showTopMembers()`** — same two passes over `memberList` (count per member).
- [x] **Step 8.20 — `showReports()`**
...
- [x] **Step 8.21 — `showMenuReport()`** — 6-option submenu (1 Borrowed→`xemDanhSachDangMuon`, 2 Overdue, 3 Popular, 4 Top members, 5 Stats, 6 Return).
- [x] **Step 8.22 — Wire into `showMenu()`** — case 4.

- [x] **✅ Module 8 done when (test against the hardcoded starter data):**
...
- [x] **Step 9.1 — Add imports + file-path constants**
  - ▸ Do:
...
- [x] **Step 9.2 — `saveData()`: the orchestrator**
  - ▸ Do:
...
- [x] **Step 9.3 — `saveBooks()`: write each book, detect its type**
  - ▸ Do:
...
- [x] **Step 9.5 — `saveBorrowings()`: dates + the null case**
  - ▸ Do:
...
- [x] **Step 9.6 — Add the "Save to file" menu option**
...
- [x] **✅ Module 9 done when:** after adding a book then choosing Save, all three `data/*.txt` files exist and contain your data in readable form (open them in Notepad to prove it).
...
- [x] **Step 10.1 — `loadData()`: the orchestrator**
  - ▸ Do:
...
- [x] **Step 10.2 — `loadBooks()`: read lines, rebuild the right subclass**
  - ▸ Do:
...
- [x] **Step 10.5 — Add the "Load from file" menu option**
  - ▸ Do: add `6. Load from file` → `case 6: loadData();`.
  - ▸ Check: choosing 6 prints the "Data loaded: N books…" summary.

- [x] **✅ Module 10 done when:** Save → exit → relaunch → Load shows the *same* data you saved. Edit a number inside `books.txt` by hand, Load, and confirm the change appears — proof it's truly reading the file.
...
- [x] **Step 11.1 — Auto-load on startup (first run uses hardcoded data, later runs load)**
...
- [x] **Step 11.2 — Auto-save on exit + new exit number**
...
- [x] **Step 11.3 — Stop the confirm-prompt crash (exception hardening)**
...
- [x] **✅ Module 11 done when:** delete `data/` → run → add a book → exit (auto-saves) → relaunch → your book is still there; and typing a letter at any confirm prompt never crashes the program.
...
- [x] Module 4 — `Borrowing` (Steps 4.1–4.11)
- [x] Module 5 — skeleton + helpers (Steps 5.1–5.17)
- [x] Module 6 — Book management (Steps 6.1–6.11)
- [x] Module 7 — Member management (Steps 7.1–7.8)
- [x] Module 8 — Borrow/Return + Reports (Steps 8.1–8.22)
- [x] Module 9 — Save to file / WRITE (Steps 9.1–9.6) · *Milestone 4*
- [x] Module 10 — Load from file / READ (Steps 10.1–10.5) · *Milestone 4*
- [x] Module 11 — Wire-up & robustness (Steps 11.1–11.3) · *Milestone 4*
